...
impl Create<'_> {
    ...
    pub async fn execute(&self, pool: &SqlitePool) -> Result<i64, sqlx::Error> {
        let result = sqlx::query!(
            "INSERT INTO feed_tokens (user_id, token_hash, label) VALUES (?, ?, ?)",
            self.user_id,
            self.token_hash,
            self.label,
        )
...
/// Resolve a feed token hash to its owning user ID.
...
impl GetUserIdByHash<'_> {
    ...
    pub async fn execute(&self, pool: &SqlitePool) -> Result<Option<i64>, sqlx::Error> {
        let row = sqlx::query_scalar!(
            "SELECT user_id FROM feed_tokens WHERE token_hash = ?",
            self.token_hash
        )
        .fetch_optional(pool)
        .await?;
...
/// List all feed tokens for a user.
...
/// Delete a feed token by ID, scoped to the owning user.
pub struct Delete {
    pub id: i64,
    pub user_id: i64,
}
...
impl Delete {
    ...
    pub async fn execute(&self, pool: &SqlitePool) -> Result<bool, sqlx::Error> {
        let result = sqlx::query!(
            "DELETE FROM feed_tokens WHERE id = ? AND user_id = ?",
            self.id,
            self.user_id,
        )
        .execute(pool)
...
mod tests {
    ...
    #[tokio::test]
    async fn feed_token_crud() {
        let pool = test_pool().await;
        let user_id = test_user(&pool, "alice").await;

        let token_id = Create {
            user_id,
            token_hash: "feed_hash_1",
            label: "my calendar",
        }
        .execute(&pool)
        .await
        .expect("feed token create failed");
        assert!(token_id > 0);
        ...
        .execute(&pool)
        .await
        .expect("feed token lookup failed")
        .expect("feed token should resolve");
        ...
        .execute(&pool)
        .await
        .expect("feed token delete failed");
        assert!(deleted);
    ...
    #[tokio::test]
    async fn feed_token_delete_scoped_to_user() {
        let pool = test_pool().await;
