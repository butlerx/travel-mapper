...
impl MultiFormatResponse for ShareTokenResponse {
    const HTML_TITLE: &'static str = "Share Token Created";
...
pub async fn create_handler(
    ...
    let parsed =
        match crate::server::extractors::FormOrJson::<ShareTokenRequest>::parse(&headers, &body) {
            Ok(v) => v,
            Err(err) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn create_share_token_returns_token() {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri("/auth/share-tokens")
                    .header(header::CONTENT_TYPE, "application/json")
                    .header(header::COOKIE, cookie)
                    .body(Body::from(r#"{"label":"my stats"}"#))
        ...
        assert_eq!(parsed["label"], "my stats");
    }

    #[tokio::test]
    async fn delete_share_token_returns_no_content() {
        let pool = test_pool().await;
        let cookie = auth_cookie_for_user(&pool, "alice").await;
        ...
        let response = app2
            .oneshot(
                Request::builder()
                    .method("DELETE")
                    .uri(format!("/auth/share-tokens/{id}"))
                    .header(header::COOKIE, cookie)
                    .body(Body::empty())
                    .expect("failed to build request"),
            )
    ...
    #[tokio::test]
    async fn delete_nonexistent_share_token_returns_not_found() {
        let pool = test_pool().await;
        let cookie = auth_cookie_for_user(&pool, "alice").await;
        let app = create_router(test_app_state(pool));
